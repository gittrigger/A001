...
public class DataProvider extends ContentProvider {
    ...
    private static String mainuri = "com.dullcomputers";
    ...
    static{
        ...
        URI_MATCHER.addURI(mainuri, "mail", ALL_MESSAGES);
        URI_MATCHER.addURI(mainuri, "mail/#", SPECIFIC_MESSAGE);
        URI_MATCHER.addURI(mainuri, "event", ALL_MESSAGES);
        URI_MATCHER.addURI(mainuri, "event/#", SPECIFIC_MESSAGE);
        URI_MATCHER.addURI(mainuri, "contact", ALL_MESSAGES);
        URI_MATCHER.addURI(mainuri, "contact/#", SPECIFIC_MESSAGE);
        URI_MATCHER.addURI(mainuri, "console", ALL_MESSAGES);
        URI_MATCHER.addURI(mainuri, "console/#", SPECIFIC_MESSAGE);
        URI_MATCHER.addURI(mainuri, "search", ALL_MESSAGES);
        URI_MATCHER.addURI(mainuri, "search/#", SPECIFIC_MESSAGE);
    }
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db){
        	...
        	try{
        		...
        		SQL += ", remoteid text unique not null";
        		SQL += ", title text, sender text, sent DATE, msgstatus text, priority text, attcnt INTEGER default 0, hp text, seenurl text, js01 text, js02 text, msgsize text";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
        		db.execSQL( SQL );
        		db.execSQL("create index mail_sender on mail(sender);");
        		db.execSQL("create index mail_sent on mail(sent);");
        		db.execSQL("create index mail_remoteid on mail(remoteid);");
        		
        		SQL = "create table event (";
        		SQL += "_id integer primary key autoincrement ";
        		//SQL += mContext.getString(R.string.contactStore_columns);
        		SQL += ", remoteid text unique not null";
        		SQL += ", title text, subtitle text, startdate DATE, enddate DATE, hp text, seenurl text";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
                db.execSQL( SQL );
                db.execSQL("create index event_startdate on event(startdate);");
                db.execSQL("create index event_enddate on event(enddate);");
                db.execSQL("create index event_remoteid on event(remoteid);");
        		...
        		//SQL += mContext.getString(R.string.contactStore_columns);
        		SQL += ", msgstatus text";
        		SQL += ", msgtitle text, msgtext text, rowtable text, rowid integer";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
                db.execSQL( SQL );
                db.execSQL("create index console_created on console(created);");
                
                SQL = "create table contact (";
        		SQL += "_id integer primary key autoincrement ";
        		//SQL += mContext.getString(R.string.contactStore_columns);
        		SQL += ", remoteid text unique not null";
        		SQL += ", pname text, email text, phone text, title text, company text, hp text, seenurl text";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
                db.execSQL( SQL );
                db.execSQL("create index contact_pname on contact(pname);");
                db.execSQL("create index contact_remoteid on contact(remoteid);");
                //db.execSQL("create index contact_reached on contact(reached);");
                
                SQL = "create table search (";
        		SQL += "_id integer primary key autoincrement ";
        		//SQL += mContext.getString(R.string.contactStore_columns);
        		SQL += ", requesturl text";
        		SQL += ", request text, filled DATE, hp text";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
        		...
        		//SQL += mContext.getString(R.string.contactStore_columns);
        		SQL += ", requesturl text";
        		SQL += ", profile text, filled DATE, hp text, uri text";
        		SQL += ", created DATE";
        		SQL += ", updated DATE";
        		SQL += ", status INTEGER default 1"; // < 0 deleted(value * -1), 1 active(created), ++ per update 
        		SQL += ");\n";
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion){
            ...
            db.execSQL("DROP TABLE IF EXISTS " + "consolehistory");
	...
	public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
		...
		//Log.i(TAG,"query() uri("+uri+") lastsegment("+uri.getLastPathSegment()+") selection("+selection+")");
